...
def crawl():
    main_site_html = request_manager.retrieve_main_html()
    latest_semester = parse_latest_semester(main_site_html)

    semester_list_html = request_manager.retrieve_semester_list_html(latest_semester)
    category_list = parse_category_list(semester_list_html)

    for category in category_list:
        print(category)
        detail_html = request_manager.retrieve_detail_html(latest_semester, category)
        parse_course_details(detail_html)

        time.sleep(0.1)

    global all_course_details
    write_json = open("data/course_content_%s.json" % get_timestamp(), "wb")
