...
class UserProfile(AutoSaveModel):
    ...
    # store json string sent from frontend
...
def create_user_profile(sender, instance, created, **kwargs):
    if created:
        UserProfile.objects.create(
            user=instance, major="", avatar="", schedule="")
...
class CourseTab(AutoSaveModel):
    ...
    # constraint: {"prerequisite": , "mutex": , "na_to_..": }
...
class ClassScheduleTab(AutoSaveModel):
    ...
    # which teaching weeks does it take place
    weeks = models.CharField(max_length=128)
    create_time = models.PositiveIntegerField()
    update_time = models.PositiveIntegerField()
...
class ConfigTab(AutoSaveModel):
    id = models.BigAutoField(primary_key=True)
    # to be specified in constants.py later on
    key = PositiveBigIntegerField(db_index=True)
