...
from scrapers.utils import get_date_time
...
def parse_category_list(semester_html):
	...
	options = soup.find("select", attrs={"name": "s_course"}).find_all("option")
...
def parse_course_details(detail_html):
	...
	description_fonts = soup.find_all("font", attrs={"size": "2", "color": None})
	assert len(blue_fonts) == 3 * len(description_fonts), "Parsing Error! Number of Course Codes/Title/AU/Description does not match!"
	...
	au_list = [au.split()[0] if len(au.split()) >= 2 else "0.0" for au in au_list]
	description_list = [font.text.strip() for font in description_fonts]

	global all_course_details
	for idx, description in enumerate(description_list):  # each table contains info of a course
		course_detail = {
...
def get_latest_semester():
	...
	for semester in parse_semester_list(main_site_html):
		semester_site_html = request_manager.get_course_content_graduate_semester_html(semester=semester)
...
def crawl(semester=None):
	...
	for category in category_list:
		detail_html = request_manager.get_course_content_graduate_detail_html(semester=semester, category=category)
