...
def _is_index_row(first_3_cells_of_row):
    cells_not_empty = map(lambda td: td.text !=
                          NULL_TD_VALUE, first_3_cells_of_row)
...
def get_course_vacancy_by_request(course_code):
    ...
    first_3_cells_of_all_rows = map(lambda x: x.findAll("td")[
                                    :3], all_rows_except_header)
    first_3_cells_of_index_rows = filter(
        _is_index_row, first_3_cells_of_all_rows)
    first_3_texts_of_index_rows = map(lambda x: list(
        map(lambda y: y.text.strip(), x)), first_3_cells_of_index_rows)
    vacancies_dict = {
