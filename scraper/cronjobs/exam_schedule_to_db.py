import datetime
from scraper.constants import VALID_COURSE_NUMBER_LOWER_BOUND
from scraper.scraper_exam_schedule import crawl_exam_schedule
...
def record_exam_schedule(course_code, exam_schedule):
    ...
    result = ExamScheduleTab.objects.filter(course_id=course_id).update(
        start_time=start_time, end_time=end_time)
    if not result:
        ExamScheduleTab.objects.create(
            course_id=course_id, start_time=start_time, end_time=end_time)
